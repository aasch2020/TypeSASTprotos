...
export async function transform(root: string): Promise<Map<string, any>> {
	...
	const adjEntries = Object.keys(roleConfig.roles || {}).map(role => {
		const arr = (roleConfig.roles[role].subsumes || []).map(a => `"${a}"`).join(", ");
		return `  "${role}": [${arr}]`;
	}).join(",\n");
	const rolesGenerated = `export type Adj = {
${adjEntries}
};

export type Role = keyof Adj;

declare const ROLE_BRAND: unique symbol;
type Brand<K> = { readonly [ROLE_BRAND]: K };
export type RoleToken<R extends Role> = Brand<R> & { readonly role: R };
export function mkRole<R extends Role>(r: R): RoleToken<R> { return ({ role: r } as unknown) as RoleToken<R>; }

export type Reachable<
  From extends Role,
  To extends Role,
  Visited extends Role = never
> =
  [From] extends [To] ? true :
  From extends Visited ? false :
  Adj[From] extends readonly (infer Ns)[] ?
